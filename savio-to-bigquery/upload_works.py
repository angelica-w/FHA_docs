'''
Load OpenAlex Work files from own machine (in this case Savio) to BigQuery

OpenAlex stores files JSON Lines format
⇒ parse each file depending on newlines and tabs
⇒ loaded to BigQuery as CSV table with single "work" column, where each cell has the JSON object of each work
...
table_id = "works"                      # change name depending on type of files being loaded

parent_directory = "PATH_TO_DIRECTORY_OF_WORK_FILES"         # path to parent directory with files

schema = [
    bigquery.SchemaField("work", "STRING"),         # change schema name depending on type of files being loaded
]
...
# specify job to read each file as a CSV with tab deliminator and append all files to a single "works" BigQuery table
